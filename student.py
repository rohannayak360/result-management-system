...
# column 2
        ...
        lbl_a_date=Label(self.root, text="Admission", font=("goudy old style", 15, "bold"), bg="white").place(x=360, y=180)
        lbl_course=Label(self.root, text="Course", font=("goudy old style", 15, "bold"), bg="white").place(x=360, y=140)

# Entry field for column 2
        self.course_list=[]
        ...
        txt_a_date=Entry(self.root,textvariable=self.var_a_date ,font=("goudy old style", 15, "bold"), bg="lightyellow").place(x=480, y=180,width=200)
        self.txt_course = ttk.Combobox(self.root,
                                     ...
                                     state='readonly',
                                     justify=CENTER)
        self.txt_course.place(x=480, y=140, width=200)
...
# -----------------------Text Address-----------------------------------------
        ...
        lbl_search_roll = Label(self.root, text="Roll No", font=("goudy old style", 15, "bold"), bg="white").place(x=700, y=60)
        txt_search_roll= Entry(self.root,textvariable=self.var_serach ,font=("goudy old style", 15, "bold"), bg="lightyellow").place(x=850, y=60,width=180)
        ...
        columns=("roll", "name", "email", "gender", "dob","contact","admission","course","state","city","pin","address"),xscrollcommand=scrollx.set, yscrollcommand=scrolly.set)
...
# Save data
    def get_data(self, ev):
        ...
        # Map correct indices to student fields
        self.var_roll.set(row[0])          # Roll No
        self.var_name.set(row[1])          # Name
        self.var_email.set(row[2])         # Email
        self.var_gender.set(row[3])        # Gender
        self.var_dob.set(row[4])           # DOB
        self.var_contact.set(row[5])       # Contact
        self.var_a_date.set(row[6])        # Admission
        self.var_course.set(row[7])        # Course
        self.var_state.set(row[8])         # State
        self.var_city.set(row[9])          # City
        self.var_pin.set(row[10])          # Pin
...
# search
    ...
    def search(self):
        ...
        cur = con.cursor()
        try:
            cur.execute(f"SELECT * FROM student WHERE roll LIKE '%{self.var_serach.get()}%'")
            rows = cur.fetchall()
            self.CourseTable.delete(*self.CourseTable.get_children())
            if len(rows) > 0:
                for row in rows:
